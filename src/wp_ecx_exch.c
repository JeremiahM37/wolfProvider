    ...
    size_t* secLen, size_t secSize)
{
    ...
    if (ok && (secret == NULL)) {
        *secLen = CURVE25519_KEYSIZE;
        ...
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG(WP_LOG_DEBUG, "wc_curve25519_shared_secret failed with rc=%d", rc);
            ok = 0;
        }
        if (ok) {
    ...
    if (ok && (secret == NULL)) {
        *secLen = CURVE448_KEY_SIZE;
        ...
        rc = wc_curve448_shared_secret(wp_ecx_get_key(ctx->key),
            wp_ecx_get_key(ctx->peer), secret, &len);
        if (rc != 0) {
            WOLFPROV_MSG_DEBUG(WP_LOG_DEBUG, "wc_curve448_shared_secret failed with rc=%d", rc);
            ok = 0;
        }
        if (ok) {
            word32 i;
